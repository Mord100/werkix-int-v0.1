...
exports.getUserFittings = asyncHandler(async (req, res) => {
  ...
  if (await isAdmin(req)) {
    ...
    const fittings = await Fitting.find(query)
      ...
      .populate({
        path: 'customer', 
        select: 'name email phone address' // Include more user details
      });
      
    // Transform fittings to include customer details
    const transformedFittings = fittings.map(fitting => ({
      ...fitting.toObject(), // Convert Mongoose document to plain object
      customerName: fitting.customer.name,
      customerEmail: fitting.customer.email,
      customerPhone: fitting.customer.phone,
      customerAddress: fitting.customer.address
    }));
      
    return res.json(transformedFittings);
  }
